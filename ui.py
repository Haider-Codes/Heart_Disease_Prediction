...
This a Machine learning web app on stream-lit that will help people to know chances of suffering from heart disease.
This app can let anyone with minimal knowledge of technology to get how vulnerable are they to any heart disease.
Anyone can use this web app, all they must do is to enter their medical and personal details and press proceed, then results will be shown within seconds.
The project predicts the likelihood of patients getting heart disease. 
It enables significant knowledge, e.g., relationships between medical factors related to heart disease and patterns, to be established.

    """)
if ch == 'view data':
    st.subheader('View dataset of patients')
    df = pd.read_csv('heart.csv')
    df.sex = df.sex.apply(lambda n: 'Male' if n==1 else 'Female')
    df.target = df.target.apply(lambda n: 'Heart Patient' if n==1 else 'Healthy')
...
if ch == 'view visualization':
    ...
    if submenu == 'patient distribution count':
        ...
        st.pyplot(fig)
    if submenu == 'patients with chest pain':
        fig,ax =plt.subplots()
